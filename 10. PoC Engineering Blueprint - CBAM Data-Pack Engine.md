...
|---|---|---|---|---|
| **Doc-intelligence VLM** | **Qwen2.5-VL-7B-Instruct** | 3B for edge; 32B/72B for accuracy ceiling | Apache-2.0 | Best-in-class structured JSON from invoices/tables/forms; strong **Traditional-Chinese**; native bbox+attribute JSON; fits 12 GB @ 4-bit |
| VLM alternative | **MiniCPM-V 2.6 (8B)** | benchmark head-to-head in Sprint 1 | model license (research/comm.) | Strong OCR, small footprint |
| Pure-OCR fallback | **PaddleOCR (PP-StructureV2)** / **GOT-OCR 2.0** | for dense tabular bill regions | Apache-2.0 | Deterministic OCR to backstop VLM on numbers |
| **Doc pre-processing** | **Docling** (IBM) | PDF/scan → structured layout | MIT | Robust layout, tables, reading order before/with the VLM |
| **Local serving (PoC)** | **Ollama** (`qwen2.5vl:7b`, incl. `:7b-q4_K_M`) | one-command local, LINE-friendly | MIT | Fast to stand up, on-prem = local-first story |
...
Qwen2.5-VL is repeatedly cited as the 2026 default for invoice/form/table extraction with stable JSON output and explicit CJK strength; it is on Ollama in 3B/7B/32B/72B (incl. `q4_K_M`), and vLLM documents `guided_json` for schema-locked decoding. MiniCPM-V 2.6 is the strongest small alternative. We run a **Sprint-1 bake-off** (Qwen2.5-VL-7B vs MiniCPM-V 2.6, ± Docling pre-pass, ± PaddleOCR numeric backstop) on the mock corpus and pick by field-level extraction accuracy on Taipower bills + MOF e-invoices.
...
```
   ...
   photo of bill/invoice │  LINE Messaging API + voice │
                         └──────────────┬─────────────┘
                                        │ images + answers
              ┌─────────────────────────▼──────────────────────────┐
              │ Ingestion: Docling layout → Qwen2.5-VL (Ollama/vLLM)│
...
- **Sprint 0 (days 1–3) — lock the target.** Download C1–C5, C8. Unzip C2, transcribe the *screws & nuts* workbook into `schema/cbam_template_map.yaml` + golden expected outputs. Pull MOENV EF DB via Swagger into `/data/ef/`. Stand up repo, FastAPI skeleton, Ollama + `qwen2.5vl:7b`.
- **Sprint 1 (week 1–2) — ingestion.** Build the 3-firm mock corpus (§9). VLM bake-off (Qwen2.5-VL-7B vs MiniCPM-V 2.6, ±Docling, ±PaddleOCR). Ship bill-parser + e-invoice-parser → validated JSON with confidences.
...
3. `ollama pull qwen2.5vl:7b`; stand up the FastAPI + LINE webhook skeleton.
...
CBAM template & examples, methodology (2025/2547), default values (2025/2621 + Excel), verification (2025/2546), guidance (EN/ZH), Q&A, registry (2024/3210), downstream proposal (52025PC0989), certificate price — all under `taxation-customs.ec.europa.eu` / `eur-lex.europa.eu` (see §3). Taiwan: MOENV EF `data.gov.tw/dataset/28176` + `data.moenv.gov.tw/swagger`; product CFP `#8992`; GHG `#16059`; Taipower generation `#8931`/`#37331`; Taipower rate tables `taipower.com.tw/2289/2290/46940`; e-invoice MIG/Turnkey open API `github.com/phidiassj/TaiwanEInvoiceOpenAPI`. Stack: Qwen2.5-VL (`huggingface.co/Qwen/Qwen2.5-VL-7B-Instruct`, `ollama.com/library/qwen2.5vl`), MiniCPM-V 2.6, Docling, PaddleOCR/GOT-OCR2, vLLM, OR-Tools, openpyxl, LINE Messaging API.
