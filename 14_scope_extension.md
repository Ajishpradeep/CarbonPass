# THE PROVABILITY DIVIDE — background annex

> 🟡 **STATUS: BACKGROUND ANNEX — NOT THE LIVE THESIS. The live direction is
> [`docs/16_waste_extension.md`](16_waste_extension.md).**
>
> **Why this was demoted (17 Jul 2026, same day it was written).** This document is a *legislation
> deck*. It argues about nations, regulations and the EU's accreditation gateway, and proposes policy
> (cluster verification) to the Commission. The competition theme is **"Digital Inclusion in the AI
> Era" — ages, regions, needs** — and **there is no person anywhere in this document.** A jury does not
> score a nation's trade advantage, and the Presidential Hackathon wants a producible product, not a
> think-tank brief. Owner's verdict: *"I am not here to tell how Taiwan run their government."*
>
> **What is still good and reusable** (all verified, all reproducible via `scripts/atlas_scan.py`):
> the **33/87 book split** (§2 claim 9, §5), the **four accreditation doors** (§2 claim 4 — re-fetched
> and parsed from the Commission's own file, totals row `24 11 7 4`), the **"same factory, moved"**
> table (§3), the measured refutation of *"developing countries get worse defaults"* (§5), and the
> Taiwan-is-the-lab-not-the-victim correction (§4). Cite these freely as **evidence**; do not revive
> the **thesis** or the policy proposal (§6 item 4).
>
> One finding here **directly feeds** `docs/16`: §5's observation that coverage is patchy *per sector*
> was the thread that led to scanning all 244 of Taiwan's goods — which found that Taiwan's **stainless
> is the worst-rated on earth** while its carbon steel is unremarkable. That is `docs/16` §4.
