...
Firm-level data never leaves the device (local-first, docs/archive/21 §1.4). What CAN
...
def export_rows(rows: list[BenchmarkRow], out_path: str | Path) -> Path:
    ...
    if bad:
        raise KAnonymityError(
            ...
            f"k-anonymity floor is a hard rule (docs/archive/21 §1.4); collect more firms "
